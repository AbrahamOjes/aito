...
const Modal = ({ closeModal, setAlert, updateAdmins, admin }) => {
  ...
  const fetctRoles = () => {
    try {
      axios.get("http://localhost:5000/admin/roles").then(res => {
        setRoles(res.data);
        setIsLoading(false);
      });
    } catch (error) {
  ...
  const submitForm = async e => {
    ...
    if (name === "") {
      ...
      setAlert("Role is required", "red-400");
      setIsLoading(false);
      return;
    }
      ...
      <div className="modal center-pos w-full lg:w-1/2 h-76 rounded shadow bg-white">
        ...
        <div className="body w-full h-4/5 px-5 flex flex-col pt-2">
          <div className=" w-full px-6 mb-2">
            <Steps current={0}>
              <Step title="In Progress" description="Admin Personal Details" />
              <Step title="Waiting" description="Admin Location" />
            </Steps>
          </div>
          <div className=" w-full px-6 mb-2">
            <label
