...
const ForgetPassword = () => {
  ...
  const onSubmitPhone = async e => {
    ...
    try {
      ...
      const body = JSON.stringify(formEmail);
      const res = await axios.post("/admin/forgot-password", body, config);
      setResetForm(false);
  ...
  const onSubmitToken = async e => {
    ...
    try {
      ...
      const res = await axios.post("/admin/verify-token", body, config);
      setResetForm(false);
      setVerificationForm(false);
      setChangeForm(true);
      // console.log(res.data.token);
      setFormChange({ ...formChange, adminId: res.data.token.user });
      setIsLoading(false);
    } catch (error) {
      // console.error(error.response.data.errors[0].msg);
      setErrors(error.response.data.errors[0].msg);
  ...
  const onSubmitPassword = async e => {
    ...
    try {
      ...
      const res = await axios.post(`/admin/change-password`, body, config);
      setResetForm(false);
      setVerificationForm(false);
      setChangeForm(true);
  ...
  return (
    <div className="user-adlogin w-full max-w-sm center-pos mt-5 mb-2 px-4">
