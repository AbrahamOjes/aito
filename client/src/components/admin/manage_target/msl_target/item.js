...
const Item = ({ target, deleteTarget, admin, setAlert, targetUpdated }) => {
  ...
  for (let i = 0; i < products.length; i++) {
    children.push(<Option key={products[i].name}>{products[i].name}</Option>);
  }
  ...
  const submitForm = async e => {
    ...
    try {
      ...
      const res = await axios.patch(
        `http://localhost:5000/admin/msl-target/${target._id}`,
        body,
        config
      );
      // console.log(res);
      ...
      setAlert("MSL Target Updated", "green-400");
      setIsLoading(false);
    } catch (error) {
      console.error(error);
        ...
        <div className="bg-white rounded hover:shadow-md w-full h-full text-gray-700 px-3 py-1 relative">
          ...
          <p className="font-normal text-sm mb-0">
            ...
            {target.products.map(product => {
              return (
                <span className="mr-1" key={product._id}>
                  {product},
                </span>
              );
            })}
