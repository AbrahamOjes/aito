...
const Modal = ({ closeModal, setAlert, updateTargets, admin }) => {
  ...
  const { region, country, area, territory, target } = formData;
  ...
  const submitForm = async e => {
    ...
    try {
      ...
      const res = await axios.post(
        "http://localhost:5000/admin/submission-target",
        body,
        config
      );
      // console.log(res);
      updateTargets(res.data.submissionTarget);
      setAlert("Submission Target Added", "green-400");
      setIsLoading(false);
      closeModal();
    } catch (error) {
      console.error(error);
