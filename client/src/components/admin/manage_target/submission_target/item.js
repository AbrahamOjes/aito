...
const Item = ({ target, deleteTarget, admin, setAlert, targetUpdated }) => {
  ...
  const { territory, form_target } = formData;
  ...
  const submitForm = async e => {
    ...
    try {
      ...
      const res = await axios.patch(
        `http://localhost:5000/admin/submission-target/${target._id}`,
        body,
        config
      );
      // console.log(res);
      ...
      setAlert("Submission Target Updated", "green-400");
      setIsLoading(false);
    } catch (error) {
      console.error(error);
    ...
    <Fragment>
          ...
          <div className="absolute opacity-25" style={{ top: 15, right: 150 }}>
            <span>
              <svg
                ...
                viewBox="0 0 24 24"
                className="w-6 icon-click-target"
              >
                <path
                  className="primary"
      ...
      >
        ...
        <div className="flex flex-col lg:flex-row">
          <div className="lg:w-2/5 w-full lg:px-2 px-0">
            <label
          ...
          <div className="lg:w-1/5 w-full lg:px-2 px-0 mt-6">
            {isLoading ? (
