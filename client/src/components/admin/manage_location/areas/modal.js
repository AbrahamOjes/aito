...
const Modal = ({ closeModal, setAlert, updateAreas }) => {
  ...
  const { name, region, country } = formData;
  ...
  const submitForm = async e => {
    ...
    try {
      ...
      const res = await axios.post(
        "http://localhost:5000/admin/area",
        body,
        config
      );
      // console.log(res);
      updateAreas(res.data.area);
      setAlert("area Added", "green-400");
      setIsLoading(false);
      closeModal();
    } catch (error) {
      console.error(error);
    ...
    <div className="modal-backdrop transparent fixed top-0 left-0 button-0 h-screen w-full z-100">
      <div className="modal center-pos w-full lg:w-1/2 h-76 rounded shadow bg-white">
        ...
        <div className="footer w-full bg-blue-050 -mb-2 px-5 py-3 text-gray-700 rounded">
          {isLoading ? (
            <button className="bg-blue-400 px-8 rounded">
              <img
