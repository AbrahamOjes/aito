...
const Modal = ({ closeModal, setAlert, updateRegions }) => {
  ...
  const submitForm = async e => {
    ...
    try {
      ...
      const res = await axios.post(
        "http://localhost:5000/admin/region",
        body,
        config
      );
      // console.log(res);
      updateRegions(res.data.region);
      setAlert("Region Added", "green-400");
      setIsLoading(false);
      closeModal();
    } catch (error) {
      console.error(error);
        ...
        <div className="body w-full h-4/5 px-5 flex flex-col pt-12">
          <div className="w-full px-6 mb-4">
            ...
            <div class="inline-block relative w-full">
              <select
                class="block appearance-none text-gray-700 w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline"
                name="country"
                value={country}
