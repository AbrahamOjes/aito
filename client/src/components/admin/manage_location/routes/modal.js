...
const Modal = ({ closeModal, setAlert, updateRoutes }) => {
  ...
  const { name, region, country, area, territory } = formData;
  ...
  const submitForm = async e => {
    ...
    try {
      ...
      const res = await axios.post(
        "http://localhost:5000/admin/route",
        body,
        config
      );
      // console.log(res);
      updateRoutes(res.data.route);
      setAlert("Route Added", "green-400");
      setIsLoading(false);
      closeModal();
    } catch (error) {
      console.error(error);
