...
const Content = ({ setAlert }) => {
  ...
  const fetchSubCategories = () => {
    try {
      axios.get("http://localhost:5000/admin/sub-categories").then(res => {
  ...
  const deleteSubCategory = id => {
    try {
      ...
      const UpdatedSubCategories = subCategories.filter(subCategory => {
        return subCategory._id !== id;
      });
      setSubCategories(UpdatedSubCategories);
      setAlert("Sub-Category Deleted", "blue-400");
      setIsLoading(false);
    } catch (error) {
      console.log(error);
    }
