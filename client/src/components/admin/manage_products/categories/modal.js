...
const Modal = ({ closeModal, setAlert, updateCategories }) => {
  ...
  const submitForm = async e => {
    ...
    setIsLoading(true);
    if (name === "") {
      setAlert("Category name is required", "red-400");
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      // console.log(res);
      updateCategories(res.data.category);
      setAlert("Category Added", "green-400");
      setIsLoading(false);
      closeModal();
    } catch (error) {
      console.error(error);
