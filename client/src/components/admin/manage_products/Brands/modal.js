...
const Modal = ({ closeModal, setAlert, updateBrands }) => {
  ...
  const submitForm = async e => {
    ...
    setIsLoading(true);
    if (name === "") {
      setAlert("Brand name is required", "red-400");
      setIsLoading(false);
      return;
    }else if(company === ""){
    ...
    try {
      ...
      const res = await axios.post(
        "http://localhost:5000/admin/brand",
        body,
        config
      );
      // console.log(res);
      updateBrands(res.data.brand);
      setAlert("Brand Added", "green-400");
      setIsLoading(false);
      closeModal();
    } catch (error) {
      console.error(error);
