...
const Modal = ({ closeModal, setAlert, updatecompanies }) => {
  ...
  const submitForm = async e => {
    ...
    setIsLoading(true);
    if (name === "") {
      setAlert("company name is required", "red-400");
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      // console.log(res);
      updatecompanies(res.data.company);
      setAlert("company Added", "green-400");
      setIsLoading(false);
      closeModal();
    } catch (error) {
      console.error(error);
